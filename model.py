...
def reshape_for_broadcast(freqs_cis: torch.Tensor, x: torch.Tensor):
    ...
    assert freqs_cis.shape == (
        x.shape[1], x.shape[-1]), f'freqs_cis.shape {freqs_cis.shape} != (x.shape[1], x.shape[-1]) {(x.shape[1], x.shape[-1])}'
...
class Llama3(nn.Module):
    ...
    def Sampler(
        ...
        # dim=-1 is the vocab_size dimension that we calculate along
        ...
        top_ks_mask = torch.arange(
            probs_idx.shape[-1], device=probs_idx.device)
        ...
        probs = torch.gather(probs_sort, dim=-1,
    ...
    def generate(
        ...
        # defaults to full max_seq_len**2 memory use. must be power of 2
        memory_saver_div: int = 1,
        temperature: float = 0.6,  # default value in meta's code
        top_p: float = 0.9,  # default value in meta's code
        top_k: int = tokenizer.n_words,  # meta's code doesn't bother with topk
    ) -> str:
        """ Wrapper around sampler() that deals with manipulation of the sequence """
        assert ((memory_saver_div & (memory_saver_div-1)) == 0) & (memory_saver_div >
                                                                   0), f'memory_saver_div {memory_saver_div} must be power of 2'
        ...
        # jic we need to add a batch dimension
